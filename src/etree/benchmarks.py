...
def run_recovery_case(case: RecoveryCase, top_k: int = 5) -> RecoveryResult:
    """Run one benchmark case and emit a single experiment row."""
    start = perf_counter()
    ...
    regime = "symbolic_target" if case.target is not None else "numeric_factory"
    domain_config = _domain_config(case)
    domain_config_text = (
        f"x_min={domain_config['x_min']:.12g};"
        f"x_max={domain_config['x_max']:.12g};"
        f"num_points={domain_config['num_points']}"
    )
